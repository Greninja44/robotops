...
- Tests: 110 fast (`pytest -m "not ros"`), 19 live-ROS (`-m ros`).
...
- The live-ROS test suite (`-m ros`) was last run in full before the hardening changes (18 integration tests passed); it has not been re-run in full since.
...
## NEXT
- Re-run the full live-ROS suite on the final code; record a real backup video (`docs/RECORDING.md`).
