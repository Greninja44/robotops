...
| Technical execution | Real rclpy tools, explicit state machine in which `REPAIRING → RESOLVED` is unrepresentable, single-use approvals, independent verification of the whole robot, 161 tests, measured benchmarks, a profiled 92 s → 10.6 s latency reduction with the cause identified (model prose, not ROS). |
